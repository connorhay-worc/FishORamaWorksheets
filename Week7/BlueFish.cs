...
public class BlueFish : IDraw
{
    ...
    public void Update()
    {
        ...
        // if (xPosition > (screen.width / 2) - (width / 2) || xPosition < (screen.width / 2) * -1 + (width / 2))
        // {
        //     xDirection *= -1;
        //     //    horizontalBumps++;
        // }
        bool swimmingUp = yDirection == 1;
        ySpeed = swimmingUp ? 4 : 2;
        
        if (yPosition > (screen.height / 2) - (height / 2) || yPosition < (screen.height / 2) * -1 + (height / 2))
        {
            yDirection *= -1;
        }
