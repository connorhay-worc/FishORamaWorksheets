...
namespace FishORama
{
    ...
    public class Simulation : IUpdate, ILoadContent
    {
        ...
        Random rand;
        OrangeFish orangeFish1, orangeFish2, orangeFish3;
        BlueFish blueFish1;
        ...
        public void LoadContent(IGetAsset pAssetManager)
        {
            ...
            initXpos = rand.Next((screen.width / 2 * -1) + (orangeFishWidth / 2), screen.width / 2 + 1 - (orangeFishWidth / 2));
            initYpos = rand.Next((screen.height / 2 * -1) + (orangeFishHeight / 2), screen.height / 2 + 1 - (orangeFishHeight / 2));
            orangeFish1 = new OrangeFish("OrangeFish", initXpos, initYpos, screen, tokenManager, rand);
            kernel.InsertToken(orangeFish1);

            initXpos = rand.Next((screen.width / 2 * -1) + (orangeFishWidth / 2), screen.width / 2 + 1 - (orangeFishWidth / 2));
            initYpos = rand.Next((screen.height / 2 * -1) + (orangeFishHeight / 2), screen.height / 2 + 1 - (orangeFishHeight / 2));
            orangeFish2 = new OrangeFish("OrangeFish", initXpos, initYpos, screen, tokenManager, rand);
            kernel.InsertToken(orangeFish2);

            initXpos = rand.Next((screen.width / 2 * -1) + (orangeFishWidth / 2), screen.width / 2 + 1 - (orangeFishWidth / 2));
            initYpos = rand.Next((screen.height / 2 * -1) + (orangeFishHeight / 2), screen.height / 2 + 1 - (orangeFishHeight / 2));
            orangeFish3 = new OrangeFish("OrangeFish", initXpos, initYpos, screen, tokenManager, rand);
            kernel.InsertToken(orangeFish3);
